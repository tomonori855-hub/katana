...
- `KatanaManager` — テーブル登録・クエリ・rebuild の中央レジストリ
- `KatanaServiceProvider` — Laravel サービスプロバイダ（auto-discovery 対応）
- `Katana` Facade
...
- `RebuildCommand` (`katana:rebuild`) — artisan コマンド
- `RebuildCacheJob` — 非同期キャッシュ再構築ジョブ
- `whereRowValuesIn` — MySQL ROW constructor IN 構文の Katana 独自拡張
