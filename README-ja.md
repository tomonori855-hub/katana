...
| `where('country','JP')` — インデックス `=`（ヒット率 20%）| **132 µs** | **1.26 ms** | **14.67 ms** |
| `where('country','JP')->where('category','electronics')` — composite index（ヒット率 2%）| **101 µs** | **933 µs** | **10.60 ms** |
| `whereBetween('price', [50,100])` — 範囲インデックス（ヒット率 25%）| **177 µs** | **1.58 ms** | **17.83 ms** |
| `where('country','JP')->orderBy('price')` — インデックスウォーク | **180 µs** | **1.58 ms** | **20.53 ms** |
| `where('active', true)` — 非インデックス全走査（ヒット率 67%）| 475 µs | 4.62 ms | 52.21 ms |
| `get()` — 全件取得 | 382 µs | 3.78 ms | 39.52 ms |
| キャッシュ構築（`rebuild()`）| 2.91 ms | 10.63 ms | 108.81 ms |
